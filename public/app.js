...
Object.assign(copy.en, { title: "Many robots. One perspective.", lede: "See the fleet. Follow the evidence. A shared workspace for understanding every robot, across every vendor.", liveOperations: "ALJAZARI / ROBOT INTELLIGENCE", navOverview: "Overview", navFleet: "Robot library", navEvidence: "Evidence desk", navMissions: "Mission ledger", monitorOnly: "MONITORING ONLY", sessionTitle: "THE OPERATIONS STUDIO", explore: "Explore the fleet", specimen: "ROBOT STUDY / 01", illustration: "Illustrative model · not hardware geometry", selectedRobot: "SELECTED ROBOT", openPassport: "Open passport ↗", topology: "A place for every robot", topologyEyebrow: "INTERACTIVE FLEET ATLAS", evidence: "The evidence desk", registry: "Your robot library", analyticsScope: "Mission metrics cover the full demo fleet; registry filters do not change this observation window.", compareHint: "Choose two robots to compare their measured signals.", clearComparison: "Clear selection", compare: "Compare", selectCompare: "Add to comparison", selectedCompare: "Selected for comparison", footerSafety: "Independent prototype · SIMULATED DATA · No physical control", pause: "Pause updates", resume: "Resume updates", refresh: "Refresh", paused: "Display paused", live: "Auto-refresh · 5s", loading: "Loading telemetry…", offline: "Refresh failed. Displaying the last snapshot; retry Refresh.", batteryLabel: "Battery", networkLabel: "Network latency", motorLabel: "Motor current", temperatureLabel: "Motor temperature", observedLabel: "Observed at (UTC)", comparisonTitle: "Evidence, side by side.", comparisonSafety: "Snapshot comparison, not a ranking. Missing capabilities remain unavailable. Different models have different operating envelopes. Health is a documented heuristic, not a failure probability.", atlasNote: "Grouped by site, not physical position. Select a robot to update its passport.", robotCount: "robots", filteredScope: "Filtered fleet", metricLabel: "Signal", close: "Close evidence", searchPlaceholder: "ID, model, client, site" });
Object.assign(copy.ar, { title: "روبوتات متعددة. رؤية واحدة.", lede: "شاهدي الأسطول، وتتبّعي الأدلة. مساحة مشتركة لفهم كل روبوت، مهما اختلفت الشركة المصنّعة.", liveOperations: "الجزري / ذكاء الروبوتات", navOverview: "نظرة عامة", navFleet: "مكتبة الروبوتات", navEvidence: "مكتب الأدلة", navMissions: "دفتر المهام", monitorOnly: "مراقبة فقط", sessionTitle: "استوديو العمليات", explore: "استكشفي الأسطول", specimen: "دراسة روبوت / ٠١", illustration: "مجسّم توضيحي · لا يمثل هندسة الجهاز", selectedRobot: "الروبوت المختار", openPassport: "افتحي ملف الروبوت ↗", topology: "مكان لكل روبوت", topologyEyebrow: "أطلس الأسطول التفاعلي", evidence: "مكتب الأدلة", registry: "مكتبة الروبوتات", analyticsScope: "مؤشرات المهام تخص الأسطول التجريبي كاملًا؛ مرشحات السجل لا تغيّر نافذة الرصد.", compareHint: "اختاري روبوتين لمقارنة الإشارات المقاسة.", clearComparison: "مسح الاختيار", compare: "مقارنة", selectCompare: "إضافة للمقارنة", selectedCompare: "مختار للمقارنة", footerSafety: "نموذج مستقل · SIMULATED DATA · دون تحكم فيزيائي", pause: "إيقاف التحديث مؤقتًا", resume: "استئناف التحديث", refresh: "تحديث", paused: "العرض متوقف مؤقتًا", live: "تحديث تلقائي · ٥ ثوانٍ", loading: "جارٍ تحميل التليمترية…", offline: "تعذّر التحديث. تُعرض آخر لقطة؛ أعيدي المحاولة بزر تحديث.", batteryLabel: "البطارية", networkLabel: "تأخير الشبكة", motorLabel: "تيار المحرك", temperatureLabel: "حرارة المحرك", observedLabel: "وقت الرصد (UTC)", comparisonTitle: "الأدلة، جنبًا إلى جنب.", comparisonSafety: "مقارنة لقطة، وليست ترتيبًا للأفضل. تبقى القدرات المفقودة غير متاحة. تختلف ظروف التشغيل بين الموديلات؛ الصحة مؤشر بقواعد موثقة وليست احتمال عطل.", atlasNote: "التجميع حسب الموقع وليس إحداثيات فعلية. اختاري روبوتًا لتحديث ملفه.", robotCount: "روبوتات", filteredScope: "الأسطول المرشّح", metricLabel: "الإشارة", close: "إغلاق الأدلة", searchPlaceholder: "المعرّف، الموديل، العميل، الموقع" });

let language = "en";
try { language = localStorage.getItem("robotops-language") === "ar" ? "ar" : "en"; } catch { /* Private browsing can deny storage. */ }
let snapshot = null;
...
function robotCard(robot) {
  return `<div class="robot-specimen" role="listitem"><button class="robot-row" type="button" data-id="${escapeHtml(robot.id)}">
    ...
    <span class="robot-mini" aria-hidden="true"></span><h3>${escapeHtml(robot.model)}</h3><p>${escapeHtml(robot.clientName)} / ${escapeHtml(robot.siteName)}</p>
...
function renderSelection() {
  ...
  document.querySelector('#specimen-subtitle').textContent = robot ? `${robot.siteName} · ${t(robot.operationalState)}` : '';
...
function tone(state) { return ({ working: "#69419b", idle: "#7b7187", warning: "#885214", critical: "#a82c53" })[state] || "#6b6079"; }
function debounce(fn, delay = 250) { let timer; return (...args) => { clearTimeout(timer); timer = setTimeout(() => fn(...args), delay); }; }

document.querySelector("#language").addEventListener("click", () => {
  language = language === "en" ? "ar" : "en";
  try { localStorage.setItem("robotops-language", language); } catch { /* Storage is optional. */ }
  render();
});
